...
Begin VB.Form POS_OtherPaymentFrm1 
   BackColor       =   &H00FFC0C0&
   ...
   End
End
Attribute VB_Name = "POS_OtherPaymentFrm1"
...
Private Sub btnAccept_Click()
    Dim due As Double
    Dim cash, check As Double
    due = Val(Replace(lblAmountDue.Caption, ",", ""))
    check = Val(Replace(txtCheckAmount.text, ",", ""))
    cash = Val(Replace(txtCash.text, ",", ""))
    If (cash + check) < due Then
    ...
    Else
        ...
        If Val(Replace(txtCheckAmount.text, ",", "")) > 0 Then
            'SAVE CHECK REGISTRY
            Set cmd = New ADODB.Command
            cmd.ActiveConnection = con
            cmd.CommandType = adCmdStoredProc
            ...
            cmd.Parameters.Append cmd.CreateParameter("@Payment", adDecimal, adParamInput, , Val(Replace(txtCheckAmount.text, ",", "")))
        ...
        cmd.CommandText = "POS_Payment_Insert"
        cmd.Parameters.Append cmd.CreateParameter("@POS_SalesId", adInteger, adParamInput, , POS_SalesId)
        cmd.Parameters.Append cmd.CreateParameter("@PaymentTypeId", adInteger, adParamInput, , 1) 'CASH
        cmd.Parameters.Append cmd.CreateParameter("@Payment", adDecimal, adParamInput, , Val(Replace(txtCash.text, ",", "")))
                              cmd.Parameters("@Payment").NumericScale = 2
                              cmd.Parameters("@Payment").Precision = 18
        ...
        POS_ChangeFrm.txtChange.text = FormatNumber((cash + check) - due, 2, vbTrue, vbFalse)
...
Private Sub txtCheckAmount_LostFocus()
    txtCheckAmount.text = FormatNumber(txtCheckNumber.text, 2, vbTrue)
End Sub
